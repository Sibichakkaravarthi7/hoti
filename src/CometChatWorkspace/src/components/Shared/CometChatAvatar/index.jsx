...
class CometChatAvatar extends React.Component {
  ...
  setAvatarImage = () => {
    if (this.props.image.trim().length) {
      ...
      if (this.props.user.hasOwnProperty('metadata')) {
        const avatarImage = this.props.user.metadata.profile_image;
    ...
    return (
      <div style={{ position: 'relative', width: '36px', height: '36px' }}>
        <img
          ...
          height={'36px'}
        />
        {this.props?.user?.metadata?.user_type && (
          <Text
            whiteSpace={'nowrap'}
            bg="white"
            p="1px 3px"
