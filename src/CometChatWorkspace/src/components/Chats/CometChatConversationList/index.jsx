...
import {
  ...
  contactSearchStyle
} from '../../Users/CometChatUserList/style';
...
class CometChatConversationList extends React.Component {
  ...
  handleSearchChange = (event) => {
    ...
    const filteredValues = this.state.conversationlist?.filter((value) =>
      value?.conversationWith?.name?.toLowerCase().includes(searchTerm)
    );
    this.setState({ searchTerm, filteredChats: filteredValues });
  };

  // giveChatList = () => {
  //   if (this.state.filteredChats?.length > 0) {
  //     return this.state.filteredChats;
  //   } else {
  ...
  render() {
    const conversationList = (
      this.state.searchTerm == '' ? [...this.state.conversationlist] : [...this.state.filteredChats]
