...
import BackButton from './resources/chat-back.svg';
...
class CometChatMessageHeader extends React.Component {
  ...
  render() {
    ...
    const iswebView = localStorage.getItem('from_webview');

    return (
      <div css={chatHeaderStyle(this.context)} className="chat__header">
        <div css={chatDetailStyle()} className="chat__details">
          <div
            css={chatSideBarBtnStyle(BackButton, this.props, this.context)}
          ...
          <div css={chatUserStyle(this.context)} className={chatWithClassName}>
            ...
            <Flex flexWrap={'wrap'} gap={'10px'}>
              <Box>{typing ? typing : status}</Box>
              <Link
                to={MAKE_PROFILE_PAGE_URL(
                  this.context.item.metadata?.user_type,
                  this.context.item.metadata?.id
                )}
                onCli
                role="button"
                fontSize={['10px', '13px']}
                ...
                {iswebView ? (
                  ...
                  <Box fontSize={['10px', '13px']} fontWeight={600}>
                    View Profile
                  </Box>
                )}
              </Link>
