...
const InfluencerFilter = ({ applyBtn, locationMeta, categoryMeta, setFilters, filters, setLocationText, resetFilter }: { applyBtn: any, locationMeta: any, categoryMeta: any, setFilters: any, filters: any, setLocationText: any, resetFilter: any }) => {

    // console.log("locationMeta ||", locationMeta )

    return (
        <motion.div initial={{ transform: "translateY(2%)" }} animate={{ transform: "translateY(0%)" }}>
            <Stack spacing={[1, 5]} style={{ padding: "1.5em", boxShadow: "2px 7px 10px rgba(21, 21, 21, 0.102)" }}>
                ...
                <AppFormFieldWrapper htmlFor={''} label={'Gender'}>
                    <CheckboxGroup value={filters?.gender} onChange={(val) => setFilters({ ...filters, gender: val })}>
                        <Stack>
                            {['Male', 'Female', 'Other'].map(each =>
                                <Checkbox variant={"filter"} color={"#585858"} value={each} key={each}><Text fontSize="15px" fontWeight={500}>{each}</Text></Checkbox>)}
                        </Stack>
                    </CheckboxGroup>
                    {filters?.gender?.length ? <Box top="9px" left="-10px" position={"absolute"} w="7px" h="7px" borderRadius={"100%"} bg={appColors.appPrimary[600]}></Box> : ""}
                </AppFormFieldWrapper>
                <Divider />
                <AppRangeContainer id={'followers'} setFilters={setFilters} filters={filters} label={'Age'} />
                <Divider />
                <Button mt="0px" onClick={() => applyBtn()} w="100%">Apply</Button>
            </Stack >
        </motion.div>
